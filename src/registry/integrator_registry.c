...
} BallisticsIntegratorRegistryEntry;

struct BallisticsIntegratorRegistry
{
    BallisticsIntegratorRegistryEntry *entries;
    size_t count;
    size_t capacity;
};

BallisticsStatus ballistics_integrator_registry_create(
    size_t capacity, BallisticsIntegratorRegistry **out_registry)
{
    ...
    *out_registry = NULL;
    if (capacity == 0U ||
        capacity > SIZE_MAX / sizeof(BallisticsIntegratorRegistryEntry))
    {
        return BALLISTICS_STATUS_INVALID_ARGUMENT;
    }
...
void ballistics_integrator_registry_destroy(BallisticsIntegratorRegistry *registry)
{
    if (registry != NULL)
    {
...
BallisticsStatus ballistics_integrator_registry_register(
...
BallisticsStatus ballistics_integrator_registry_find(
...
BallisticsStatus ballistics_integrator_registry_count(
...
BallisticsStatus ballistics_integrator_registry_entry_identifier(
...
BallisticsStatus ballistics_integrator_registry_create_instance(
    ...
    const void *config, size_t config_size, BallisticsIntegrator **out_object)
{
    ...
    *out_object = NULL;
    status = ballistics_integrator_registry_find(registry, identifier, &factory);
    if (status != BALLISTICS_STATUS_OK)
    {
        return status;
    }
