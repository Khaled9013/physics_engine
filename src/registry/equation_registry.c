...
} BallisticsEquationRegistryEntry;

struct BallisticsEquationRegistry
{
    BallisticsEquationRegistryEntry *entries;
    size_t count;
    size_t capacity;
};

BallisticsStatus ballistics_equation_registry_create(
    size_t capacity, BallisticsEquationRegistry **out_registry)
{
    ...
    if (out_registry == NULL || capacity == 0U ||
        capacity > SIZE_MAX / sizeof(BallisticsEquationRegistryEntry))
    {
...
void ballistics_equation_registry_destroy(BallisticsEquationRegistry *registry)
{
    if (registry != NULL)
    {
...
BallisticsStatus ballistics_equation_registry_register(
...
BallisticsStatus ballistics_equation_registry_find(
...
BallisticsStatus ballistics_equation_registry_count(
...
BallisticsStatus ballistics_equation_registry_entry_identifier(
...
BallisticsStatus ballistics_equation_registry_create_instance(
    ...
    const void *config, size_t config_size, BallisticsEquation **out_object)
{
    ...
    *out_object = NULL;
    status = ballistics_equation_registry_find(registry, identifier, &factory);
    if (status != BALLISTICS_STATUS_OK)
    {
        return status;
    }
